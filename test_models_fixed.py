...
class ModelTester:
    ...
    def _sanitize_for_metrics(self, targets: np.ndarray, probabilities: np.ndarray):
        """Sanitize arrays before computing metrics to avoid NaN/inf errors.
        - Coerces to float arrays
        - Replaces NaN in targets with 0 (treated as absent label)
        - Replaces NaN/inf in probabilities and clips to [0,1]
        Returns (targets_clean, probabilities_clean)"""
        t = np.array(targets, dtype=float, copy=True)
        p = np.array(probabilities, dtype=float, copy=True)

        # Replace NaNs in targets with 0, and any non-binary values are thresholded at 0.5
        t = np.nan_to_num(t, nan=0.0, posinf=1.0, neginf=0.0)
        # If targets aren't strictly 0/1, binarize
        t = (t > 0.5).astype(int)

        # Replace NaNs/infs in probabilities and clamp to [0,1]
...
def run_inference(self, test_loader) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        with torch.no_grad():
            for batch in tqdm(test_loader, desc="Processing batches"):
                ...
                # Sanitize per-batch to prevent NaNs propagating
        ...
        thr = np.array(self.thresholds, dtype=float)
        if thr.ndim == 0:
            thr = np.full(all_probabilities.shape[1], float(thr))
        all_predictions = (all_probabilities >= thr).astype(int)
...
def calculate_average_precision(self, targets: np.ndarray, probabilities: np.ndarray) -> Dict[str, float]:
        """Calculate Average Precision for each category (NaN-safe)"""
        # Sanitize globally first
        targets, probabilities = self._sanitize_for_metrics(targets, probabilities)
        ...
        emotion_names = [
            ...
            'Yearning', 'Disapproval', 'Aversion', 'Anger', 'Sensitivity',
            'Sadness', 'Disquietment', 'Fear', 'Pain', 'Suffering'
        ]

        for i, emotion in enumerate(emotion_names):
            # Build a valid mask for this column only
            t_i = targets[:, i]
            p_i = probabilities[:, i]
            valid = np.isfinite(t_i) & np.isfinite(p_i)
            if valid.sum() == 0 or t_i[valid].sum() == 0:
                ap_scores[emotion] = float('nan')
                continue
            try:
                ap = average_precision_score(t_i[valid], p_i[valid])
            except Exception:
                # Fallback safety: if sklearn still complains, replace any remaining bad values
                t_safe = np.nan_to_num(t_i[valid], nan=0.0)
                p_safe = np.nan_to_num(p_i[valid], nan=0.0)
                ap = average_precision_score(t_safe, p_safe)
            ap_scores[emotion] = float(ap)
